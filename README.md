...
Utilizamos o **Antigravity CLI** integrado diretamente ao **VS Code** para orquestrar 21 especialidades previdenciárias de forma autônoma e precisa.
...
2.  **21 Skills Especializadas:** Cada regra legal (B41, B42, CI, Rural, Facultativo, etc.) possui sua própria Skill no Antigravity CLI, garantindo assertividade matemática e legal.
3.  **Orquestrador (`/calcula`):** Comando central que identifica o seu pedido e aciona o especialista correto.

## 📁 Estrutura do Projeto (.agents)

- `.agents/skills/`: Contém as 21 ferramentas especializadas (ex: `skill-ci-20-1007`, `skill-facultativo-upgrade-1686`).
- `.agents/workflows/`: Ponto de entrada do comando `/calcula` e fluxos.
